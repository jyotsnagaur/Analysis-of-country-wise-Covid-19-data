...
Q2.3 marks) To build a visualisation using the ggplot2 library, we use the following template:
...
c)geometry, introduced by the function geom() that takes care of drawing the graph (bar graph, line, scatter plot, etc). There can be different geometries like horizontal,vertical,diagonal lines, barplot,contour,density plots,point,boxplot,jitter,polygon etc.
...
3.  The correlation coefficient is unit-less. The value will stay very similiar even with scaling or normalising any of the variables. The coeeficient between x and y is same as the correlation coefficient between y and x. 





Q4. (3 marks) Imagine we have a dataset that lists the heights of the fathers and their sons. You have built a linear model that encodes the relationship between the fathers’ heights and the sons’ heights as follows:
