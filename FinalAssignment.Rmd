...
```{r}
new_recov2 <- as.data.frame(new_recov)  # converting to data frame
head(new_recov2)
```
...
master_df6 <- master_df5 %>%
  group_by(Country) %>%
  mutate(Cases_1M_Pop = CumCases * (10^6) / Population, Deaths_1M_Pop = CumDeaths * (10^6) / Population, Recovered_1M_Pop = CumRecovered * (10^6) / Population, Tests_1M_Pop = CumTests * (10^6) / Population)
head(master_df6)
```
...
deaths_per_day <- master_df6 %>%
  group_by(Date) %>%
  summarise(death_toll = sum(NewDeaths))
...
continent_data <- lastDay_data %>%
  group_by(Continent) %>%
  summarise(case_con = sum(CumCases), death_con = sum(CumDeaths), recov_con = sum(CumRecovered), test_con = sum(CumTests))
...
```{r}
plot_covid_test <- test_country_detail %>% pivot_longer(
  cols = c(infected_total, total_tests, total_test_per_million),
...
```{r}
c_plot <- continent_data %>% pivot_longer(
  cols = c(case_con, death_con, recov_con, test_con),
...
ggplot(cor_data, aes(CumCases)) +
  geom_histogram(aes(y = ..density..), fill = "aquamarine3", bins = 50) +
  xlim(c(0, 1200000)) #+ylim()#normalise data for y-axis
