...
```{r}
master_df5 <- master_df4 %>% group_by(Country) %>% mutate(Active=(CumCases-(CumDeaths + CumRecovered)), FatalityRate=(CumDeaths / CumCases))
head(master_df5)
```
...
```{r}
continent_data <- lastDay_data %>% group_by(Continent) %>% summarise(case_con=sum(CumCases), death_con=sum(CumDeaths), recov_con=sum(CumRecovered), test_con=sum(CumTests))
...
coun <- top10casesW$Code

CumCases_with_time <- master_df6 %>% filter(Code %in% coun)

head(CumCases_with_time)
...
coun2 <- top10activeW$Code

Active_with_time <- master_df6 %>% filter(Code %in% coun2)

head(Active_with_time)
```
...
```{r}
#Preparing the data
coun3 <- top10testsMW$Code

Test_with_time <- master_df6 %>% filter(Code %in% coun3)
...
```{r}
test_country_detail <- Test_with_time %>% group_by(Country) %>% summarise(infected_total=max(CumCases), total_tests=max(CumTests), total_test_per_million=max(Tests_1M_Pop))

test_country_detail
```

```{r}
plot_covid_test <-test_country_detail %>% pivot_longer(cols = c(infected_total, total_tests, total_test_per_million),
...
p3 = ggplot(plot_covid_test,aes(x = Country, 
                     y = value, 
                     col = Test_Detail, 
                     group = Test_Detail,
                     fill= Test_Detail)) +
...
```{r}
c_plot <- continent_data%>% pivot_longer(cols = c(case_con, death_con, recov_con,test_con),
...
#library(gghighlight)
p4 = ggplot(c_plot,aes(x = Continent, 
                     y = value, 
                     col = Confirmed_Data, 
                     group = Continent,
                     fill= Continent, alpha=0.9)) +
...
```{r}
library(tidyverse)
c_data <-lastDay_data %>% select(CumCases, CumTests, Population, GDP, GDPCapita)
cor_data<- c_data[-1]
cor_data
```
...
p + geom_boxplot() + facet_wrap(~key, scale="free") # viewing sepaartely as the scale of the variables is different
```
...
ggplot(cor_data, aes(CumCases)) +
  geom_histogram(aes(y = ..density..), fill = "aquamarine3", bins =50) +xlim(c(0,1200000))#+ylim()#normalise data for y-axis
...
-  In the <b>first Linear Regression model</b> we built, we were assuming that the future value of cumulative cases only depended on the GDP of that country. According to our assumption: The Cumulative Cases and GDP have a linear relationship with each other. The Root Mean Square Error of 1st model was 76591.77. This is a very high value of error and proves that the model is inaccurate or the variables need to be changed into log values to extract a linear relationship.
