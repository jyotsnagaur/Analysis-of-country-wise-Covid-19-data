...
```{r}
new_recov4<- pivot_longer(new_recov3, cols = 1:185, names_to ="location",
 values_to = "cases")
new_recov4
```
...
```{r}
recov_5<- new_recov4 %>% group_by(location) %>% arrange(location)
recov_5
```
...
class(recov_6$Date)
#country_info data frame doesnt have a Date variable. Rest in all data tibbles, the date variable is of Date type.
