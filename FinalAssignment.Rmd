...
Observation from above graph: The total cases(green) have increased exponentially over the time of 5 months. The number of tests(grey) have also increased which result in pointing towards whether the cases are infected or asymptomatic. The blue line is the recovered cases which follows a similar trend as the infected cases showing a direct co-dependence between the two. The deaths are shown in red which was actually more than the recovered cases during Jan to Feb and slowly afterwards the number of deaths became lower than number of recoveries after Feb. 
...
Observation from above graph: The UK has shown the most number of total cases on the last day as compared to all other countries.Turkey hasn't experienced/reported cases before March 2020. The infected cases for all the countries has peaked exponentially from Feb on wards.
...
Observation from above graph:The US has shown the maximum increase in total cases towards the last days.In all countries, the graphs have progressed exponentially since Mar 2020.
...
Observation from above graph:Portugal had the most number of total tests and hence the most infected cases. The total test per million was highest in Iceland.
...
Observation from above graph:The stacked graph shows the confirmed cases in each of the continents. Europe had the most number of confirmed infected cases followed by North America, Asia, South America, Oceania and Africa.
...
Observation: The highest correlation is shown between CumCases with CumTests and CumCases with GDP which is 0.9. This means in our data the Cumulative cases are highly dependent on the GDP of the country. It also means that the total number of tests are dependent on the Cumulative infected Cases which is obvious since with increase in the number of tests the infected cases will increase.
...
There are some outliers for each of the variables but that could be because of the population of the countries across the world varies drastically.
...
Observation: The distribution looks more evenly spread out when we take the log values of x-axis as compared to its original values. Perhaps because taking the log reduces the magnitude of the value. Also the y-axis in first case shows the count of each range and the y-axis of second case shows the density of each case.
...
Inference from above graph: The data is mostly concentrated and in a scatter formation on the graph. Perhaps it is not so suitable for a linear regression.The fitted line hardly touches some points so will have huge value of error.

```{r}
ggplot(lmModel, aes(lmModel1$residuals)) +
...
-  The Root Mean Square Error of 1st model was 30908. This is a very high value of error and proves that the model is inaccurate or the variables need to be changed into log values to extract a linear relationship.
...
-  The Root Mean Square Error of 2nd model was 24775.91. This is a very high value of error and proves that the model is inaccurate or the variables need to be changed into log values to extract a linear relationship.
